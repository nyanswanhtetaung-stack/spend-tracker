...
![Findings computed from the figures, and the written summary beneath them](docs/findings.png)

The red panels are arithmetic — which categories went over, by how much, and
whether one of them accounts for most of the gap. Doing that locally is
instant, free and always right, so no model is involved.

The grey box underneath is the part where a model earns its place: it reads
the *notes* on each expense and separates a one-off annual renewal from a
pattern of creeping costs. The numbers alone cannot tell you that.
